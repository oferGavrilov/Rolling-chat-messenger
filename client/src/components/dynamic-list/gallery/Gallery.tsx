...
export default function Gallery(): JSX.Element {
    ...
    return (
        <div>
            {!isLoading && <div onClick={() => setIsShowUploadForm(!isShowUploadForm)} className="cursor-pointer ml-auto text-sm mr-4 flex items-center justify-center rounded-xl px-3 py-2 w-max bg-gray-300/20 hover:text-primary">
                {isShowUploadForm ? 'Back To Gallery' : 'Add New Image'}
            </div>}
            ...
            {!isShowUploadForm ?
                (
                    <div className='grid grid-cols-2 gap-2 mx-4 my-6'>
                        {gallery.map((item) => (
                            <div key={item._id} className={`gallery-item ${selectedImage?._id === item._id && 'outline-2 outline outline-offset-2 outline-primary opacity-80'} ${isLoading && 'opacity-45 pointer-events-none animate-pulse'}`} onClick={() => onSelectImage(item)}>
                                ...
                                <div className="remove-btn" onClick={(ev) => onRemoveItem(ev, item)}>
                    ...
                    <UploadGalleryImage setIsShowUploadForm={setIsShowUploadForm} />
                )}
