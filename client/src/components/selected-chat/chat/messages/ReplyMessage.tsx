...
export default function ReplyMessage ({ message, loggedInUserId, incomingMessage }: Props): JSX.Element {
      ...
      return (
            <div className={`${incomingMessage ? 'ml-auto dark:bg-dark-incoming-chat-bg' : 'dark:bg-dark-outgoing-chat-bg'} text-white rounded-t-xl p-2 w-full`}>
                  <div className={`flex flex-col text-sm py-2 pl-4 pr-1 rounded-md border-r-4 ${message.replyMessage?.sender._id === loggedInUserId ? 'border-lime-400' : 'border-[#ffb703]'} ${incomingMessage ? 'bg-[#828995] dark:bg-dark-incoming-chat-reply-bg' : 'bg-[#0079ca] dark:bg-dark-outgoing-chat-reply-bg'}`}>
                        {getSender()}
