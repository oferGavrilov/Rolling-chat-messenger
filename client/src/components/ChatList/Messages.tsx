...
export default function Messages ({ setShowSearch }: Props) {
      ...
      return (
            <div className="pt-7 relative">
                  ...
                  <BsSearch size={16} className="absolute top-[6rem] left-4  text-[#00000085]" />

                  <AiOutlineArrowDown
                        onClick={() => setSearch('')}
                        size={20}
                        className={`absolute top-[6rem] right-3 text-primary opacity-0 pointer-events-none cursor-pointer ${search ? 'custom-rotate pointer-events-auto' : 'reverse-rotate'}`} />
